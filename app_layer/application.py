...
def load_image_to_array(path: str, modality: str, validate_mode: bool = True) -> np.ndarray:
    ...
    - segmentation: RGB (IDs will be derived later) -> uint8 HxWx3
    """
    im = Image.open(path)
    if modality == "edge":
...
def _suppress_white_boundaries(rgb: np.ndarray, white_thresh: int = 250, iters: int = 2) -> np.ndarray:
    """
    Remove white-like boundary strokes by propagating neighboring non-white colors (8-neighborhood).
    Guarantees: output contains no white-like pixels per threshold, unless the whole image is white
    (that degenerate case is handled later).
    """
    H, W, _ = rgb.shape
    out = rgb.copy()
...
def _build_seg_ids_and_palette(rgb_arr: np.ndarray, white_thresh: int) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    修正点：
    * 白様色を除去したあと，旧インデックス→新インデックス（連番 0..K-1）の写像を作り，inv を再マップする．
    * これにより `palette[ids]` が常に有効になり，範囲外参照が起きない．
    """
    H, W, _ = rgb_arr.shape
    ...
    # 旧→新インデックス写像（長さ = 旧 uniq.size）
    old2new = np.full(uniq.shape[0], -1, dtype=np.int64)
    for new_i, old_i in enumerate(nonwhite_idx):
        old2new[old_i] = new_i

    # 白様色は最近傍の非白へ寄せる（RGB L2 最小）
    if white_like.any():
        white_idx = np.where(white_like)[0]
        for wi in white_idx:
            col = uniq[wi].astype(np.int32)
            d2 = np.sum((nonwhite_colors - col) ** 2, axis=1)
            new_i = int(np.argmin(d2))
            old2new[wi] = new_i  # 0..K-1

    # inv を連番に再マップ
    inv_new = old2new[inv]
    if np.any(inv_new < 0):
        inv_new = np.maximum(inv_new, 0)  # 保険

    K = nonwhite_idx.size
    if K <= 256:
        id_map = inv_new.astype(np.uint8).reshape(H, W)
        enc = "id8"
    else:
        id_map = inv_new.astype(np.uint16).reshape(H, W)
        enc = "id16"

    palette = uniq[nonwhite_idx].astype(np.uint8)  # 白様色は含まない
...
# ----------------- Semantic ID corruption (optional) -----------------
...
def serialize_content(modality: str, content_path: str, text_encoding: str="utf-8",
    ...
    if modality == "segmentation":
        ...
        _set_seg_palette(palette)  # pre-shared in-process
...
def deserialize_content(hdr: AppHeader, payload_bytes: bytes, text_encoding: str="utf-8",
    ...
    if hdr.modality == "segmentation":
        ...
        K = int(pal.shape[0])
        ids_clamped = np.minimum(ids, K - 1)  # safety under bit errors
        rgb = pal[ids_clamped]
